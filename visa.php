...
if (isset($_POST['acao']) && $_POST['acao'] == 'cad_orcamento') {
	...
	while ($n <= $valida_meses) {
		mysqli_query($_SG['conexao'], "INSERT INTO orcamento (mes,ano,valor,conta,usuario) values ('$mes','$ano','$valor_orcamento','2','$usuario')");
		echo mysqli_error($_SG['conexao']);
		header("Location: ?mes=" . $_GET['mes'] . "&ano=" . $_GET['ano']);
		if ($mes <= 11) {
			$mes++;
...
<body style="padding:10px" onLoad="initTimer();">
	...
	<table cellpadding="1" cellspacing="10" width="1000" align="center" style="background-color:#033">
		<tr>
			...
			<td colspan="2" align="right" style="background-color:#005B5B;">
				<a style="color:#FFF" href="?mes=<?php echo date('m') ?>&ano=<?php echo date('Y') ?>">Hoje:<strong> <?php echo date('d') ?> de <?php echo mostraMes(date('m')) ?> de <?php echo date('Y') ?><br></strong> Hora: <strong><span id="timer"></span></strong></a></strong>
			</td>
		</tr>
		<tr>
			<td width="70">
					...
					<table cellpadding="0" cellspacing="0" width="100%">
						...
						<tr style="display:none; background-color:#E0E0E0" id="limite_visa">
							<td>
								<form id="form_cart" method="post" action="?mes=<?php echo $mes_hoje ?>&ano=<?php echo $ano_hoje ?>">
									...
									<b>Dia vencimento:</b>
									<font color="#FF0000" size=2><input type="text" value="<?php echo $dia_venc_v ?>" name="diavenc" id="diavenc" size="1" maxlength="2"></font>
									<input type="submit" class="input" value="Gravar" />
								</form>
							</td>
						</tr>
						...
						<tr>
							...
							<td align="right"><strong style="font-size:22px; color:<?php if ($resultado_geral < 0) echo "#C00";
																					else echo "rgba(4, 45, 191, 1)" ?>"><?php echo formata_dinheiro($resultado_geral) ?></strong></td>
						</tr>
					</table>
				</fieldset>
			</td>
	...
	<table border="0" cellpadding="5" cellspacing="1" width="1000" align="center">
		...
		<tr style="display:none; background-color:#E0E0E0" id="export_pdf">
			...
			<td style="font-size:14px">
				<b>
					<center>Exclusões de movimentos
				</b><br>
				...
				<form method="post" action="./exportar/exclusoes.php">
					...
					<input type="hidden" name="conta" value="2" />
					<br><br><br><br><br><br>
					<center><input type="submit" class="input" value="Exportar" />
				</form>
			</td>
		</tr>
	</table>
	...
	<table cellpadding="5" cellspacing="0" width="1000" align="center">
		...
		<tr style="background-color:#E0E0E0">
			...
			<td align="right"><b><?php echo "Valor" ?></td>
		</tr>
		<?php
		$filtros = "";
		...
		?>
			...
			<tr style="background-color:<?php if ($cont % 2 == 0) echo "#E0E0E0";
										else echo "#F1F1F1" ?>">
				...
				<td align="right"><strong style="color:<?php if ($row['tipo'] == 0) echo "#C00";
				...
				<td colspan="3">
					...
					<form enctype="multipart/form-data" method="post" action="?mes=<?php echo $mes_hoje ?>&ano=<?php echo $ano_hoje ?>">
						...
						<select name="cat">
							...
							?>
								<option <?php if ($row2['id'] == $row['cat']) echo "selected" ?> value="<?php echo $row2['id'] ?>"><?php echo $row2['nome'] ?></option><?php } ?>
						</select><br /><br />
			...
			<tr style="display:none; background-color:<?php if ($cont % 2 == 0) echo "#F1F1F1";
														else echo "#E0E0E0" ?>" id="hist_mov_<?php echo $row['id'] ?>">
